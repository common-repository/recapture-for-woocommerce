    ...
    */
    ...
    static function get_local_currency() {
        try {
            ...
            // Check WordPress Multilingual Plugin
            if (has_filter('wcml_get_client_currency')) {
                $wcml_currency = apply_filters('wcml_get_client_currency', null);
                if ($wcml_currency) {
    ...
    function update_cart() {
        ...
        $cart = (object) [
            ...
            'currency' => self::get_local_currency(),
            'discountCodes' => WC()->cart->get_applied_coupons(),
        ];
        ...
        foreach ($woocommerce_cart as $item_key => $item) {
            ...
            $cart_items[] = [
                ...
                'price' => $price // updates if any discounts are applied
            ];
        }
        ...
        // first request, empty cart so we don't need to send it
        if (!isset($_COOKIE['ra_woo_cart_contents']) && count($cart->products) == 0) {
            return;
        }
        ...
        if (
            ...
            // set the cookie value so if we have multiple updates in a single request we only send one
            $_COOKIE['ra_woo_cart_contents'] = $cart_contents;
    ...
    static function order_completed($order_id)
    {
        ...
        foreach ($order->get_items() as $item_id => $item) {
            ...
            $price = $order->get_item_total($item, self::include_sales_tax(), true);
    ...
    public function regenerate_cart_from_url($cart, $contents)
    {
        ...
        if ($meta != null && $meta->cart != null) {
            ...
            // if we don't have meta data for this cart and the user has disabled cart recovery then exit
            if (has_filter('recapture_disable_cart_recovery_without_meta')) {
                $disable_recovery= apply_filters('recapture_disable_cart_recovery_without_meta', false);
                if ($disable_recovery == true) {
